	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				...
				<table class="table table-bordered table-responsive table-striped table-hover js-basic-example dataTable">
					...
					<tbody>
						...
						<tr>
							...
							<td><a href="{{url('admin/apparels/category/edit/'.$cats->id)}}"><i class="material-icons">edit</i></a></td>
							<td width="5px"><a href="{{url('admin/apparels/category/'.$cats->id)}}" onclick="event.preventDefault();
								document.getElementById('delete-form-{{$cats->id}}').submit();">
								<form id="delete-form-{{$cats->id}}" action="{{ url('admin/apparels/category/'. $cats->id) }}" method="post" style="display: none;">
									{{ csrf_field() }}
									{{ method_field('DELETE') }}
		...
		<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
			<div class="card">
				<div class="body">
					No data to display.
					<a href="{{url('admin/apparels/add-category')}}" class="btn btn-info pull-right">Add Category</a>
				</div>
			</div>
		</div>
		@endif
